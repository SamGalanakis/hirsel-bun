    ...
    let mut response = db
        .query(
            "LET $node = type::record('kg_node', $node_key);
             SELECT kind, node_id, label FROM $node;",
        )
...
async fn replace_document_reference_edges(
    ...
    for reference in refs {
        ...
        db.query(
            "RELATE $from->kg_edge->$to SET relation = $relation, metadata = {}, created_at = time::now();",
        )
        .bind(("from", from.clone()))
        .bind(("to", to))
...
pub async fn process_doc_edge_queue() -> Result<usize, String> {
    ...
    let mut processed = 0;
    for row in rows {
        let Some(project_id) = record_project_id(&row.id) else {
            continue;
        };
        ...
        let mut content_result = db
            .query("SELECT content FROM kg_node WHERE id[0] = $pid AND kind = 'document' AND node_id = $nid LIMIT 1")
            .bind(("pid", project_id))
        ...
        let _ = replace_document_reference_edges(project_id, &row.node_id, &refs).await;
