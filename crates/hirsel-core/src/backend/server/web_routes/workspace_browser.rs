...
async fn resolve_project_main_path(project_id: i64) -> Result<PathBuf, String> {
...
async fn resolve_root_path(project_id: i64, root_id: &str) -> Result<PathBuf, String> {
    match parse_root_id(root_id)? {
        WorkspaceRootRef::Main => resolve_project_main_path(project_id).await,
        WorkspaceRootRef::Remote => Err("remote workspace browsing is not available".to_string()),
        WorkspaceRootRef::Thread(thread_id) => {
            ...
            let path = thread
                ...
                .map(PathBuf::from)
                .ok_or_else(|| format!("thread {} has no working directory set", thread_id))?;
...
fn git_current_branch_from_dir(repo_dir: &FsPath) -> Option<String> {
