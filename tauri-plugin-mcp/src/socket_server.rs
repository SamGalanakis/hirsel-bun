...
pub struct SocketServer<R: Runtime> {
    ...
    /// Track the socket path for cleanup on Unix
...
impl<R: Runtime> SocketServer<R> {
    ...
    pub fn start(&mut self) -> crate::Result<()> {
        ...
        let listener = match &self.socket_type {
            SocketType::Ipc { path } => {
                ...
                // On Unix, clean up any stale socket file before binding
                #[cfg(unix)]
                if socket_path.exists() {
                    info!("[TAURI_MCP] Removing stale socket file: {}", socket_path.display());
                    if let Err(e) = std::fs::remove_file(&socket_path) {
                        error!("[TAURI_MCP] Failed to remove stale socket: {}", e);
                    }
                }

                // Store the path for cleanup on drop
    ...
    pub fn stop(&self) -> crate::Result<()> {
        ...
        *self.running.lock().unwrap() = false;
        ...
        if let Some(ref path) = self.socket_path {
            if path.exists() {
                info!("[TAURI_MCP] Cleaning up socket file: {}", path.display());
                if let Err(e) = std::fs::remove_file(path) {
                    error!("[TAURI_MCP] Failed to remove socket file: {}", e);
                }
            }
        }
    ...
    #[cfg(desktop)]
    fn get_socket_name(&self, path: &Option<std::path::PathBuf>) -> Result<Name<'static>, Error> {
        let socket_path = if let Some(p) = path {
            p.to_string_lossy().to_string()
        } else {
            let temp_dir = std::env::temp_dir();
...
impl<R: Runtime> Drop for SocketServer<R> {
    fn drop(&mut self) {
        // Ensure we stop the server and clean up the socket
        let _ = self.stop();
    }
}
