...
use crate::backend::config::{paths::hirsel_dir, Config};
use crate::backend::orchestrator::{LocalOrchestrator, Orchestrator};
...
pub async fn start_daemon(config: DaemonConfig) -> Result<()> {
    ...
    // Build router (same routes as the backend server but no auth for local daemon)
...
fn build_router(state: Arc<AppState>) -> Router {
    use crate::backend::server::shared_routes;
...
async fn get_run_state(
    state: &AppState,
    runtime_name: &str,
) -> Result<crate::backend::state::SQLiteState, (StatusCode, String)> {
    ...
    crate::backend::state::SQLiteState::new(runtime_name)
        .await
...
// Uses shared handlers from crate::backend::server::worker_routes
// =============================================================================

use crate::backend::server::worker_routes::{
...
) -> Option<crate::backend::lifecycle::LocalLifecycleManager> {
    let runtime_dir = config.runtimes_dir().join(runtime_name);
    let agent_command = crate::cli::config::get_agent_command();
    match crate::backend::lifecycle::LocalLifecycleManager::new(
        runtime_name,
        runtime_dir,
        agent_command,
    )
