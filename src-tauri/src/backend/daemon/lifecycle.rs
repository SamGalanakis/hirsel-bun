...
use crate::backend::api_types::RunStatus;
use crate::backend::config::{self, Config};
use crate::backend::conflict_resolver::ConflictResolverService;
use crate::backend::delta::{
    list_working_route_runtimes, BoardDeliveryStatus, Delivery, DeltaState,
};
use crate::backend::lifecycle::{
    LifecycleAction, LifecycleEvent, LifecycleManager, LocalLifecycleManager,
};
use crate::backend::orchestrator::{create_local_orchestrator, Orchestrator};
use crate::backend::scribe;
use crate::backend::server::AppState;
use crate::backend::state::{SQLiteState, Status};
    ...
    match status {
        Status::Working => {
            ...
            for worker in &workers {
                if worker.status == crate::backend::state::WorkerStatus::Working {
                    if let Some(pid) = worker.pid {
                        if !crate::backend::runner::local::LocalRunner::is_pid_alive(pid as u32) {
                            tracing::warn!(
                            ...
                            let _ = lifecycle
                                ...
                                .update_worker(
                                    &worker.name,
                                    crate::backend::state::WorkerUpdate {
                                        pid: Some(None),
                                        status: Some(crate::backend::state::WorkerStatus::Error),
            ...
            for worker in &workers {
                if worker.status == crate::backend::state::WorkerStatus::Awaiting
                    && !worker.hitl_waiting
                {
                    if let Some(pid) = worker.pid {
                        if crate::backend::runner::local::LocalRunner::is_pid_alive(pid as u32) {
        ...
        Status::Eval => {
            ...
            if let Ok(Some(eval)) = lifecycle.state().get_running_eval().await {
                if let Some(pid) = eval.pid {
                    if !crate::backend::runner::local::LocalRunner::is_pid_alive(pid) {
                        tracing::warn!(
...
async fn handle_lifecycle_actions(
    ...
    use crate::backend::delta::bump_generation;
...
async fn process_single_delivery(delivery: &Delivery, _config: &Config) -> anyhow::Result<()> {
    ...
    let git_ops = crate::backend::delivery::GitOperations::new(&work_dir);
