...
use crate::backend::delta::{BoardNode, BoardNodeStatus, DeltaState, NodeKind};
use crate::backend::files::Files;
use crate::backend::runner::{create_lifecycle_runner_for_handle, WorkerHandle};
use crate::backend::snapshot::{
    create_archive_strategy, host_session_path, AgentSnapshot, ArchiveStrategy, WorkDirSnapshot,
    WorkerStateHandle,
};
use crate::backend::state::{FailureReason, SQLiteState, Status, WorkerStatus, WorkerUpdate};
...
impl LocalLifecycleManager {
    ...
    fn generate_ephemeral_worker_name(used: &HashSet<String>) -> String {
        loop {
            let base = crate::backend::names::generate_worker_name();
