...
use crate::backend::route::{CreateRouteRequest, Route, RouteStore, UpdateRouteSettingsRequest};
use crate::backend::route_runtime::get_route_runtime_name;
...
pub async fn archive_route(project_id: i64, route_id: i64) -> Result<Route, String> {
    let store = RouteStore::new(project_id).await.str_err()?;
    let project_store = crate::backend::project::ProjectStore::open()
        .await
        .str_err()?;
...
pub async fn set_active_route(project_id: i64, route_id: i64) -> Result<(), String> {
    ...
    let pool = crate::backend::db::global_pool().await;
    sqlx::query("UPDATE projects SET active_route_id = ?, updated_at = ? WHERE id = ?")
        .bind(route_id)
        .bind(crate::backend::db::utc_now())
        .bind(project_id)
        .execute(pool)
        .await
