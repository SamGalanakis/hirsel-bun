...
use crate::backend::delivery::{delivery_branch_name, DeliveryOrchestrator};
use crate::backend::delta::{
    BoardDeliveryStatus, BoardVersion, Delivery, DeliveryAttempt, DeltaState,
};
use crate::backend::hirsel_dir;
...
fn resolve_remote(remote_url: Option<&str>) -> Option<String> {
    if let Some(url) = remote_url {
        if !url.is_empty() {
            return Some(crate::backend::system::normalise_local_remote(url));
        }
    }
    None
}

use crate::backend::system::is_local_remote;
    ...
    if has_remote {
        ...
        if target_exists_on_remote {
            if let Ok(state) = orchestrator.check_merge_state(&target_branch) {
                ...
                if state == crate::backend::state::MergeState::Conflicts {
    ...
    if is_local {
        if let Some(ref url) = effective_remote {
            ...
            if !path.exists() || (!path.join("HEAD").exists() && !path.join(".git").exists()) {
                use crate::backend::delivery::GitOperations;
