...
pub trait WorkspaceProvider: Send + Sync {
    ...
    async fn read_file_string(&self, workspace_name: &str, path: &str) -> HirselResult<String> {
        let bytes = self.read_file(workspace_name, path).await?;
    ...
    async fn write_file_string(
        ...
        self.write_file(workspace_name, path, content.as_bytes())
            .await
    }
}
