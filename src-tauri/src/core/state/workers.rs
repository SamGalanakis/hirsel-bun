...
impl SQLiteState {
    ...
    pub(super) fn worker_from_row(row: &Row) -> rusqlite::Result<Worker> {
        Ok(Worker {
            ...
            hitl_waiting: row
                ...
                .map(|v| v != 0)
                .unwrap_or(false),
            snapshot_handle: row.get("snapshot_handle")?,
        })
    }
    ...
    pub fn get_worker(&self, name: &str) -> StateResult<Option<Worker>> {
        let result = self.db.query_row(
            "SELECT id, name, pid, runner_id, runner_type, session_id, session_started_at, status, work_dir, waiting_thread, needs_restart, location, last_heartbeat, created_at, hitl_waiting, snapshot_handle FROM workers WHERE name = ?1",
            params![name],
    ...
    pub fn get_workers(&self) -> StateResult<Vec<Worker>> {
        let mut stmt = self.db.prepare(
            "SELECT id, name, pid, runner_id, runner_type, session_id, session_started_at, status, work_dir, waiting_thread, needs_restart, location, last_heartbeat, created_at, hitl_waiting, snapshot_handle FROM workers ORDER BY id"
        )?;
        let workers = stmt
    ...
    pub fn get_active_workers(&self) -> StateResult<Vec<Worker>> {
        let mut stmt = self.db.prepare(
            "SELECT id, name, pid, runner_id, runner_type, session_id, session_started_at, status, work_dir, waiting_thread, needs_restart, location, last_heartbeat, created_at, hitl_waiting, snapshot_handle FROM workers WHERE status NOT IN (?1, ?2) ORDER BY id"
        )?;
        let workers = stmt
            .query_map(
                params![
    ...
    pub fn update_worker(&self, name: &str, updates: WorkerUpdate) -> StateResult<()> {
        ...
        if let Some(ref snapshot_handle) = updates.snapshot_handle {
            set_clauses.push("snapshot_handle = ?");
            params_vec.push(Box::new(snapshot_handle.clone()));
        }
    ...
    pub fn get_hitl_waiting_workers(&self) -> StateResult<Vec<Worker>> {
        let mut stmt = self.db.prepare(
            "SELECT id, name, pid, runner_id, runner_type, session_id, session_started_at, status, work_dir, waiting_thread, needs_restart, location, last_heartbeat, created_at, hitl_waiting, snapshot_handle FROM workers WHERE hitl_waiting = 1 ORDER BY id"
        )?;
        let workers = stmt
            .query_map([], Self::worker_from_row)?
            .collect::<Result<Vec<_>, _>>()?;
