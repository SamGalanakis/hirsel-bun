...
use crate::core::constants::{MAX_VERDICT_RETRIES, VERDICT_REMINDER_PROMPT};
use crate::core::lifecycle::LocalLifecycleManager;
use crate::core::{EvalStatus, Files, SQLiteState, Status, TaskSource};
...
pub async fn run_eval_from_args(
    ...
    if result.success {
        ...
        if failed_count >= 3 {
            ...
            // Add feedback as a remediation task (Worker source - generated by eval system)
            let task_id = format!("eval_fix_{}", failed_count);
            let task_desc = format!("Fix eval failures:\n\n{}", result.feedback);
            state.add_task_with_source_simple(
                &task_id,
                &task_desc,
                None,
                None,
