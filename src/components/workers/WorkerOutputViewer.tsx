        ...
        <div class="bg-pasture-800 border border-pasture-600 rounded-lg shadow-xl w-[90vw] h-[90vh] max-w-6xl flex flex-col overflow-hidden">
          ...
          <div class="p-4 border-b border-pasture-600 flex items-center justify-between shrink-0">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <i data-lucide="brain" class="w-3.5 h-3.5 inline-block mr-1" />
                Thinking
              </button>
              {/* Toggle auto-scroll */}
              <button
              ...
              >
                <i data-lucide="arrow-down-to-line" class="w-3.5 h-3.5 inline-block mr-1" />
                Auto-scroll
              </button>
              {/* Close */}
              <button
          ...
          >
            <Show when={events.length === 0}>
              <div class="text-wool-500 text-center py-8">
                ...
                <Show when={workerStatus() !== 'working'}>
                  <i data-lucide="terminal" class="w-12 h-12 mx-auto mb-4 text-wool-600" />
                  <p>No events yet</p>
                </Show>
              </div>
            </Show>
...
const ToolBlock: Component<{
  ...
  const toolTitle = () => latestEvent()?.toolTitle || 'Tool';
  const toolKind = () => latestEvent()?.toolKind;
  const toolStatus = () => latestEvent()?.toolStatus;
  const toolInput = () => latestEvent()?.toolInput;
  const toolOutput = () => latestEvent()?.toolOutput;
  ...
  return (
    <div class="border border-pasture-600 rounded overflow-hidden">
      <button
        class="w-full px-3 py-2 flex items-center gap-2 bg-pasture-800 hover:bg-pasture-700 text-left"
        onClick={props.onToggle}
      >
        <i data-lucide={kindIcon()} class="w-4 h-4 text-wool-400" />
        <span class="flex-1 text-sm text-wool-200 truncate">{toolTitle()}</span>
        <i data-lucide={statusIcon()} class={`w-4 h-4 ${statusColor()}`} />
        <i
          data-lucide={props.expanded ? 'chevron-up' : 'chevron-down'}
          class="w-4 h-4 text-wool-500"
        />
      </button>

      <Show when={props.expanded}>
        <div class="p-3 space-y-3 bg-pasture-900 border-t border-pasture-600">
          <Show when={parsedInput()}>
            <div>
              <p class="text-xs text-wool-500 mb-1">Input</p>
              <pre class="text-xs text-wool-400 bg-pasture-800 p-2 rounded overflow-x-auto">
          ...
          <Show when={toolOutput()}>
            <div>
              <p class="text-xs text-wool-500 mb-1">Output</p>
              <pre class="text-xs text-wool-400 bg-pasture-800 p-2 rounded overflow-x-auto max-h-64 overflow-y-auto">
                {toolOutput()}
              </pre>
            </div>
          </Show>
