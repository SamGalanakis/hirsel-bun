...
const HATS: Record<number, { svg: string; y: number }> = {
  ...
  // Crown (for leaders)
  1: {
    svg: `<path d="M-6 0 L-4 -6 L-2 -2 L0 -8 L2 -2 L4 -6 L6 0 Z" fill="#FFD700" stroke="#DAA520" stroke-width="0.4"/><circle cx="-2" cy="-1.5" r="0.8" fill="#E53935"/><circle cx="0" cy="-3" r="0.8" fill="#4CAF50"/><circle cx="2" cy="-1.5" r="0.8" fill="#2196F3"/>`,
    y: -1,
  },
  // Cowboy
  2: {
    svg: `<ellipse cx="0" cy="0" rx="8" ry="1.5" fill="#8B4513"/><path d="M-5 0 Q0 -7 5 0" fill="#A0522D"/>`,
    y: 0,
  },
  // Top hat
  3: {
    svg: `<ellipse cx="0" cy="0" rx="5" ry="1.2" fill="#1a1a1a"/><rect x="-3" y="-9" width="6" height="9" fill="#1a1a1a"/><rect x="-3" y="-2" width="6" height="1.2" fill="#8B0000"/>`,
    y: 0,
  },
  // Beanie
  4: {
    svg: `<path d="M-5 0 Q-5 -6 0 -7 Q5 -6 5 0" fill="#E91E63"/><circle cx="0" cy="-8" r="2" fill="#E91E63"/>`,
    y: 0,
  },
  // Wizard
  5: {
    svg: `<path d="M0 -14 L-5 0 L5 0 Z" fill="#4A148C"/><circle cx="0" cy="-6" r="0.7" fill="#FFD700"/>`,
    y: 0,
  },
  // Chef
  6: {
    svg: `<ellipse cx="0" cy="-5" rx="5" ry="4" fill="#FAFAFA"/><ellipse cx="-3" cy="-6" rx="3" ry="3" fill="#FAFAFA"/><ellipse cx="3" cy="-6" rx="3" ry="3" fill="#FAFAFA"/><rect x="-4" y="-1" width="8" height="2.5" fill="#FAFAFA"/>`,
    y: 0,
  },
  // Hard hat
  7: {
    svg: `<path d="M-5 0 Q0 -7 5 0" fill="#FFC107"/><rect x="-5" y="-1.5" width="10" height="2" fill="#FFC107"/>`,
    y: 0,
  },
  // Detective hat (reserved for eval agents)
  8: {
    svg: `<ellipse cx="0" cy="0" rx="7" ry="1.8" fill="#4A4A4A"/><path d="M-4.5 0 Q-4.5 -5 0 -6 Q4.5 -5 4.5 0" fill="#5D5D5D"/><rect x="-5" y="-1" width="10" height="1.5" fill="#3D3D3D"/>`,
    y: 0,
  },
};
...
export function getHatName(id: number): string {
  return (
    ['None', 'Crown', 'Cowboy', 'Top Hat', 'Beanie', 'Wizard', 'Chef', 'Hard Hat', 'Detective'][
      id
    ] || 'Unknown'
  );
}
